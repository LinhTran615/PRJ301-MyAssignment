...
<c:set var="canViewAgenda" value="false" />
<c:set var="canViewRequests" value="false" />
<c:set var="canCreateRequest" value="false" />
<c:set var="canEditRequest" value="false" />

<c:forEach var="role" items="${sessionScope.auth.roles}">
    <c:forEach var="f" items="${role.features}">
        <c:choose>
            <c:when test="${f.url == '/division/agenda' 
                            || f.url == '/agenda' 
                            || fn:startsWith(f.url, '/division/agenda') 
                            || fn:startsWith('/division/agenda', f.url)}">
                <c:set var="canViewAgenda" value="true" />
            </c:when>
            <c:when test="${f.url == '/request/list'}">
                <c:set var="canViewRequests" value="true" />
            </c:when>
            <c:when test="${f.url == '/request/create'}">
                <c:set var="canCreateRequest" value="true" />
            </c:when>
            <c:when test="${f.url == '/request/edit'}">
            ...
            <tbody>
                <c:forEach var="r" items="${recentMine}">
                    <tr>
                        ...
                        <td>${r.from}</td>
                        <td>${r.to}</td>
                        <td class="text-truncate" style="max-width:260px;" title="${r.reason}">${r.reason}</td>
                        <td>
                            <c:choose>
                        ...
                        <td>
                            <c:if test="${canEditRequest
                                          and (r.status == 0 || r.status == 2)
                                          and r.created_by.id == sessionScope.auth.employee.id}">
                                  <a class="btn btn-outline" href="${pageContext.request.contextPath}/request/edit?rid=${r.id}">
